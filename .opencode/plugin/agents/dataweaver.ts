// .opencode/plugin/agents/dataweaver.ts
import type { AgentConfig } from "@opencode-ai/sdk"
import type { BuiltinAgentDefinition } from "./types"

export type DataweaverAvailableAgent = {
  name: string
  description: string
  mode?: string
}
...
export const dataweaverDefinition: BuiltinAgentDefinition = {
  name: "dataweaver",
  create(config, existingAgents, _skills) {
    const raw = existingAgents["dataweaver"] as
      | { disable?: boolean }
      | undefined
    ...
    // Don't override a user-defined dataweaver
    if (existingAgents["dataweaver"]) {
      return null
    }

    const systemDefaultModel = config.model as string | undefined
    return createDataweaverAgent(systemDefaultModel)
  },
}
